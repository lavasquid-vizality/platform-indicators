...
export const TooltipSVGComponent = memo(({ platform, status, width, height, place }) => {
  return <Tooltip aria-label={false} text={`${status === 'dnd' ? 'DND' : toTitleCase(status)} ${toTitleCase(platform)}`}>{ props => {
...
export const SpaceSVGComponent = memo(({ height }) => {
  return <svg width={'1'} height={height} className={'PI-Space'}/>;
});

export const DivComponent = memo(({ place, children }) => {
  return <div className={`PI-Div PI-Div-${place}`}>
    {children}
  </div>;
}, (prevProps, nextProps) => {
